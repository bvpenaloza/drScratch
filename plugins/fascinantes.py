...
class Fascinantes(HairballPlugin):
    """This class provides plugins for Fascinating Animate Fascinantes criterias
    ...
    def __init__(self):
        """Construct method"""
        super(Fascinantes, self).__init__()
        self.__final_result = {}

    def analyze(self, scratch, **kwargs):
        """Run and return the results from the Fascinantes plugins."""
        #Variable
        block_list = {}
        script_number = 0
    ...
    def __analyzecolores(self, block_list):
    ...
    def __analyzeartista(self, block_list):
    ...
    def __analyzeanidado(self, block_list):
        """Plugin that checks if a Scratch project contain anidados"""
        #Variable
        contain_loop = False
    ...
    def __analyzegeometricas(self, block_list):
        """Check if a Scratch project contain basic geometricass"""
        #Variable
        contain_geometric_block = False
        geometric_figure_number = 0
        #Search geometricas patterns trough loops
        for row in block_list.keys():
            for name, block in block_list[row]:
                for target in self.GEOMETRIC_LOOP_PATTERN:
                    if target in name:
                        geometric_figure_number += self.__containgeometricasPattern(block_list, block.args)
    ...
    def __containgeometricasPattern(self, block_list, argument_list):
        """Search geometricas patterns trough loops"""
        #Variable
        ...
        #Check argument of loop
            ...
            for argument in argument_list[1]:
                if isinstance(argument, kurt.Block):
                    ...
                    if isinstance(argument.type, kurt.CustomBlockType):
                        pattern = self.__containCustomgeometricasPattern(block_list, pattern, argument.type.text.replace(' %s', ''))
    ...
    def __containCustomgeometricasPattern(self, block_list, pattern, target):
        """Search geometricas patterns trough CustomBlockType"""
        for row in block_list.keys():
            ...
            if target in block_list[row][0][1].stringify():
                for name, block in block_list[row]:
                    ...
                    if isinstance(block.type, kurt.CustomBlockType) and target != block.type.text.replace(' %s', ''):
                        pattern = self.__containCustomgeometricasPattern(block_list, pattern, block.type.text.replace(' %s', ''))
        return pattern
